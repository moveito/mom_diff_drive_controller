...
#include "mom_diff_drive_controller/diff_drive_controller.hpp"
...
namespace mom_diff_drive_controller
{
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  RCLCPP_INFO(logger, "command: x:%lf, y,%lf, theta:%lf", reference_interfaces_[0],reference_interfaces_[1],reference_interfaces_[2]);
//  RCLCPP_INFO(logger, "command: linear:%lf, angular,%lf", linear_command, angular_command);
  ...
  if (should_publish)
  {
    if (realtime_odometry_publisher_->trylock())
    {
      ...
      state_interfaces_values_[0] = odometry_.getX();
      state_interfaces_values_[1] = odometry_.getY();
      state_interfaces_values_[2] = odometry_.getHeading();
      state_interfaces_values_[3] = odometry_.getLinear() * cos(odometry_.getHeading());
      state_interfaces_values_[4] = odometry_.getLinear() * sin(odometry_.getHeading());
      state_interfaces_values_[5] = odometry_.getAngular();
...
DiffDriveController::on_export_state_interfaces()
{
  const int nr_st_itfs = 6;
  state_interfaces_values_.resize(nr_st_itfs, 0.0);
  ...
  state_interfaces.push_back(hardware_interface::StateInterface(
    get_node()->get_name() + std::string("/x"), hardware_interface::HW_IF_POSITION,
    &state_interfaces_values_[0]));
  state_interfaces.push_back(hardware_interface::StateInterface(
    get_node()->get_name() + std::string("/y"), hardware_interface::HW_IF_POSITION,
    &state_interfaces_values_[1]));
  state_interfaces.push_back(hardware_interface::StateInterface(
    get_node()->get_name() + std::string("/theta"), hardware_interface::HW_IF_POSITION,
...
CLASS_LOADER_REGISTER_CLASS(
  mom_diff_drive_controller::DiffDriveController, controller_interface::ChainableControllerInterface)
